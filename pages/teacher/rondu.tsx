...
const Rondu = () => {
  const router =useRouter()
    ...
    const {data}=useSWR(`http://192.168.137.200:8000/api/teacher_exam_results/${parseInt(router?.query.devId as string)}`,fetcher)     
 
    console.log(data)


  return (
      <Box p={16}><Heading textAlign={"center"} py={16}>Rondu</Heading>
      ...
      <Tbody>
        {data?.map((item:any) => (
          <Tr key={item.id}>
            ...
            <Td>{format(new Date(item.dueDate) ,"yyyy-MM-dd HH:mm") }</Td>
            <Td>
                ...
                icon={<AiOutlineEye />}
                        variant="ghost"
                         onClick={()=>router.push(`/teacher/ronduDetail?ronduId=${item?.id}`)}
              />
            </Td>
          </Tr>
        ))}
